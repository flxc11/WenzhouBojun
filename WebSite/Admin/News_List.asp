...
Case "Up"
	...
	ClassID=ReplaceBadChar(Trim(Request("ClassID")))
	if ClassID <>0 then
		MaxOrder=Conn.Execute("Select Max(NewsOrder) From NewsInfo where ClassID="&ClassID&"")(0)
		if Cstr(NewsOrder)<>Cstr(MaxOrder) then
			NextOrder=Conn.Execute("Select top 1 NewsOrder from NewsInfo where ClassID="&ClassID&" and NewsOrder>"&NewsOrder&" order by NewsOrder")(0)
		end if
	else
		MaxOrder=Conn.Execute("Select Max(NewsOrder) From NewsInfo")(0)
		NextOrder=Conn.Execute("Select top 1 NewsOrder from NewsInfo where NewsOrder>"&NewsOrder&" order by NewsOrder")(0)
	end if
	If Cstr(NewsOrder)=Cstr(MaxOrder) Then
		Response.Write("<script>alert('\u8be5\u4fe1\u606f\u5df2\u7ecf\u5904\u4e8e\u6700\u5934\u90e8\u4e86\uff0c\u65e0\u6cd5\u8fdb\u884c\u987a\u5e8f\u7684\u8c03\u6574\u3002');history.back();</script>")
	Else
		Conn.Execute("Update NewsInfo Set NewsOrder="&NewsOrder&" Where NewsOrder="&NextOrder&"")
		Conn.Execute("Update NewsInfo Set NewsOrder="&NextOrder&" Where ID="&ID&"")
...
Case "Down"
	...
	ClassID=ReplaceBadChar(Trim(Request("ClassID")))
	if ClassID <>0 then
		MinOrder=Conn.Execute("Select Min(NewsOrder) From NewsInfo where ClassID="&ClassID&"")(0)
		if Cstr(NewsOrder)<>Cstr(MinOrder) then
			PrevOrder=Conn.Execute("Select top 1 NewsOrder from NewsInfo where ClassID="&ClassID&" and NewsOrder<"&NewsOrder&" order by NewsOrder desc")(0)
		end if
	else
		MinOrder=Conn.Execute("Select Min(NewsOrder) From NewsInfo")(0)
		PrevOrder=Conn.Execute("Select top 1 NewsOrder from NewsInfo where NewsOrder<"&NewsOrder&" order by NewsOrder desc")(0)
	end if
	If Cstr(NewsOrder)=Cstr(MinOrder) Then
		Response.Write("<script>alert('\u8be5\u4fe1\u606f\u5df2\u7ecf\u5904\u4e8e\u6700\u5e95\u90e8\u4e86\uff0c\u65e0\u6cd5\u8fdb\u884c\u987a\u5e8f\u7684\u8c03\u6574\u3002');history.back();</script>")
	Else
		Conn.Execute("Update NewsInfo Set NewsOrder="&NewsOrder&" Where NewsOrder="&PrevOrder&"")
		Conn.Execute("Update NewsInfo Set NewsOrder="&PrevOrder&" Where ID="&ID&"")
		Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	End If
	Conn.Close
	Set Conn=Nothing
...
<th width="36%" class="Right">信息标题<a href="#" onclick="top.CreateNewTab('News_ContentAdd.asp','News_ContentAdd','添加信息')">[+]</a></th>
...
<%end if%>&nbsp;</td>
<td class="Right"><a href="?Action=Up&ID=<%=Rs("ID")%>&NewsOrder=<%=Rs("NewsOrder")%>&ClassID=<%=Rs("ClassID")%>">上移</a>┊<a href="?Action=Down&ID=<%=Rs("ID")%>&NewsOrder=<%=Rs("NewsOrder")%>&ClassID=<%=Rs("ClassID")%>">下移</a></td>
<td class="Right">
<%
If Rs("NewsIndex")="0" Then
Response.Write("<a href=""?Action=UnIndex&ID="&Rs("ID")&""">推荐</a>")
Else
